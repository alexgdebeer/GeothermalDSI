...
DSI_SAMPLES = [100, 250, 500, 1000, 2000, 5000, 10_000]
...
def plot_setup(data_setup, fname, well_to_plot=3):
    ...
    forecast = patches.Rectangle(
        xy=(OBS_END, 16), width=LIMS_TS[1]-OBS_END, height=4, 
...
def plot_dsi_prior_check(data_setup, data_results, fname):

    wells_to_plot = [5, 4, 3, 6]

    _, axes = plt.subplots(2, 4, figsize=(10.0, 5.5), sharex=True, sharey=True)
    ...
    for i, well_num in enumerate(wells_to_plot):
        ...
        pri_preds = data_results["pri_preds"][well_num, :100, :].T
        dsi_preds = data_results["dsi_pri"][well_num, :100, :].T

        # pres_t = data_setup["p_wells"][well_num]
        # pres_obs = data_setup["d_obs"][well_num]

        axes[0][i].plot(ts, pri_preds, c=C_PRES, lw=1, alpha=0.5)
        axes[1][i].plot(ts, dsi_preds, c=C_PRES, lw=1, alpha=0.5)

        for ax in axes[:, i]:

            forecast = patches.Rectangle(
                xy=(OBS_END, 10), width=LIMS_TS[1]-OBS_END, height=10, 
                # linewidth=1, #edgecolor=None, 
                facecolor="gainsboro",
                zorder=1
            )
            ...
            tufte_axis(ax, bnds_x=(0, 160), bnds_y=(10, 20), 
                       xticks=(0, 80, 160), yticks=(10, 15, 20))

            ax.set_box_aspect(1)
            ax.add_patch(forecast)
            # ax.axvline(OBS_END, ls="--", c="gray", ymin=1/12, ymax=11/12, zorder=1)

        axes[0][i].set_title(get_well_name(well_num))

    axes[0][0].set_ylabel("Prior\nPressure [MPa]")
    axes[1][0].set_ylabel("DSI (Unconditional)\nPressure [MPa]")
...
def plot_results(data_setup, data_results, fname):
    ...
    for i, well_num in enumerate(wells_to_plot):
        ...
        dsi_preds = data_results["dsi_preds_1000"][well_num, :, :].T
        ...
        for ax in axes[i]:
            ...
            forecast = patches.Rectangle(
                xy=(OBS_END, 15.5), width=LIMS_TS[1]-OBS_END, height=5, 
                # linewidth=1, #edgecolor=None, 
                facecolor="gainsboro",
                zorder=1
            )
            ...
            tufte_axis(ax, bnds_x=(0, 160), bnds_y=(15.5, 20.5), 
                       xticks=(0, 80, 160), yticks=(15.5, 18, 20.5))
        ...
        axes[i][0].set_ylabel(get_well_name(well_num)+f"\nPressure [MPa]")
...
def plot_final_pressures(data_setup, data_results, fname):
    ...
    for i in range(9):
        ...
        dsi_preds = data_results["dsi_preds_1000"][well_num, :, -1]
        ...
        dsi_density = gaussian_kde(dsi_preds)(xs)
        ...
        p_truth = axes.flat[i].axvline(truth, c="k", ymin=1/22, ymax=21/22, zorder=0, label="Truth")
...
def plot_sample_comparison(data_setup, data_results, fname):
    ...
    xmins = [16.0, 16.0, 16.0, 15.0, 15.0, 15.5, 15.0, 16.0, 16.0]
    xmaxs = [17.0, 17.0, 17.0, 17.0, 16.5, 17.0, 17.0, 17.0, 17.0]
    ymin = 0
    ymaxs = [17, 14, 17, 8, 10, 10, 10, 10, 10]
    ...
    for i in range(9):
        ...
        xs = np.linspace(xmins[i], xmaxs[i], 1000)
        ...
        p_truth = axes.flat[i].axvline(truth, c="k", ymin=1/22, ymax=21/22, lw=1.2, zorder=0, label="Truth")
        ...
        for j, l in enumerate(DSI_SAMPLES):

            dsi_preds = data_results[f"dsi_preds_{l}"][well_num, :, -1]
            dsi_density = gaussian_kde(dsi_preds)(xs)
            label = f"DSI ($J={l}$)"
            p, = axes.flat[i].plot(xs, dsi_density, c=cmap[j], label=label, lw=1.2)
            
            if i == 0:
                handles.append(p)

        axes.flat[i].set_box_aspect(1)
        tufte_axis(axes.flat[i], bnds_x=(xmins[i], xmaxs[i]), bnds_y=(ymin, ymaxs[i]))
        axes.flat[i].set_title(r"$\texttt{WELL " + str(i+1) + r"}$")
...
if __name__ == "__main__":
    ...
    plot_setup(data_setup, fname="figures/fig2.pdf")
    plot_states(data_setup, fname="figures/fig3.pdf")
    plot_prior_draws(data_setup, fname="figures/fig4.pdf")
    plot_dsi_prior_check(data_setup, data_results, fname="figures/fig5.pdf")
    plot_results(data_setup, data_results, fname="figures/fig6.pdf")
    plot_final_pressures(data_setup, data_results, fname="figures/fig7.pdf")
    plot_sample_comparison(data_setup, data_results, fname="figures/fig8.pdf")