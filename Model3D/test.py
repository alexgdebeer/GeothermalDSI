...
RUN_DSI = True 
RUN_SAMPLE_COMPARISON = False


if RUN_DSI:

    transformer = CDFTransformer(GFs[:, :1400])
    Fs_pri = transformer.generate_prior_samples(ny=len(y), n=1000)
    Fs_post = transformer.generate_conditional_samples(y, n=1000)
...
if RUN_SAMPLE_COMPARISON:
    ...
    for n in n_samples:
        ...
        transformer = CDFTransformer(GFs[:, :n])
        Fs_post = transformer.generate_conditional_samples(y, n=1000)
